...
router.get('/', async function (req, res, next) {
  let result = await getContract(req.headers.username, 'itu');
...
router.get('/one/history', async function (req, res, next) {
  ...
  let response = await result.contract.evaluateTransaction('getHistoryForStudent',req.query.identity_number,'true');
  await result.gateway.disconnect();
  response = gpaCalculator(JSON.parse(JSON.parse(JSON.stringify(response.toString()))));
  res.json(response);
  console.log(`Transaction has been evaluated, result is: ${response.toString()}`);
...
router.post('/register-user', async function (req, res, next) {
  const result = await registerUser(req.headers.username, 'itu')
